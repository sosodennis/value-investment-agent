...
def calculate_ebit(revenue: list[float], operating_margins: list[float]) -> list[float]:
    return [r * m for r, m in zip(revenue, operating_margins)]

def calculate_nopat(ebit: list[float], tax_rate: float) -> list[float]:
    return [e * (1 - tax_rate) for e in ebit]

def calculate_fcff(nopat: list[float], revenue: list[float], 
                  da_rates: list[float], capex_rates: list[float], 
                  wc_rates: list[float], sbc_rates: list[float]) -> list[float]:
...
def calculate_terminal_value(final_fcff: float, wacc: float, terminal_growth: float) -> float:
    # TV = FCFF_n+1 / (WACC - g)
