...
def project_net_income(initial_net_income: float, income_growth_rates: list[float]) -> list[float]:
    income = []
...
def calculate_required_capital(rwa: list[float], tier1_target_ratio: float) -> list[float]:
    return [r * tier1_target_ratio for r in rwa]

def calculate_dividends(net_income: list[float], required_capital: list[float], initial_capital: float) -> list[float]:
...
def calculate_pv(dividends: list[float], cost_of_equity: float, terminal_growth: float) -> float:
