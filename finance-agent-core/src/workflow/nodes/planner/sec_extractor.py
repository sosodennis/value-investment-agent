...
class SearchConfig:
    ...
    dimension_regex: Optional[str] = None
...
class SearchType:
    ...
    def DIMENSIONAL(concept_regex: str, dimension_regex: str) -> SearchConfig:
        return SearchConfig(concept_regex=concept_regex, type_name="DIMENSIONAL", dimension_regex=dimension_regex)
...
class SECExtractResult(BaseModel):
    ...
    dimension_detail: Optional[dict] = Field(default_factory=dict)
...
class SECReportExtractor:
    ...
    def _load_report_data(self):
        ...
        # 智慧對齊：考慮申報時差 [1, 2]
        filings = company.get_filings(form="10-K", year=[self.fiscal_year, self.fiscal_year + 1], amendments=False)
        
        target_filing = next((f for f in filings if pd.to_datetime(f.period_of_report).year == self.fiscal_year), None)
        if not target_filing: target_filing = filings.latest()
        if not target_filing: raise ValueError(f"找不到 {self.ticker} 報告。")
        ...
        # 鎖定日期錨點
        dei_mask = self.df['concept'].str.contains('DocumentPeriodEndDate', case=False, na=False)
        if dei_mask.any():
            self.actual_date = str(self.df[dei_mask].iloc[0]['value'])[:10]
            print(f"[*] 鎖定報表截止日: {self.actual_date}")

        # 預先識別維度列
        self.real_dim_cols = [col for col in self.df.columns if 'Axis' in col or col.startswith('dim_')]
    ...
    def search(self, config: SearchConfig) -> List[SECExtractResult]:
        ...
        print(f"\n>>> 搜尋執行: '{config.concept_regex}' | 模式: {config.type_name}" + 
              (f" | 維度過濾: '{config.dimension_regex}'" if config.dimension_regex else ""))

        # 1. 標籤與日期初步過濾
        processed_regex = config.concept_regex if ":" in config.concept_regex else f".*:{config.concept_regex}$"
        mask = self.df['concept'].str.contains(processed_regex, flags=re.IGNORECASE, na=False)
        
        if self.actual_date:
            date_mask = (self.df['period_end'] == self.actual_date) | (self.df['period_key'].str.contains(self.actual_date, na=False))
            mask = mask & date_mask
        ...
        if config.type_name == "CONSOLIDATED":
            ...
            if config.dimension_regex and self.real_dim_cols:
                dim_mask = self.df[self.real_dim_cols].apply(
                    lambda x: x.astype(str).str.contains(config.dimension_regex, flags=re.IGNORECASE, na=False)
                ).any(axis=1)
                mask = mask & dim_mask
        ...
        for _, row in unique_df.iterrows():
            dim_detail = {col.split('_')[-1]: row[col] for col in self.real_dim_cols if pd.notna(row[col])}
            dim_str = "\n".join([f"{k}: {v}" for k, v in dim_detail.items()]) if dim_detail else "None (Total)"
        ...
        if final_rows:
            ...
            simple_display = table_display.drop(columns=['dimension_detail', 'dimensions'], errors='ignore')
    ...
    def debug_asset_issue(self, tag: str):
        ...
        if matches.empty:
            print("[-] 驚人！連原始數據都找不到。可能使用了不同的 Tag (如 AssetsCurrent?)")
            return

        # 2. 找出所有非空的維度欄位
        # 過濾掉全是 NaN 的列，只保留有值的維度列
        active_dim_cols = matches[self.real_dim_cols].dropna(axis=1, how='all').columns.tolist()
