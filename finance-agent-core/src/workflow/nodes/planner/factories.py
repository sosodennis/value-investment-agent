...
from .financial_models import (
    BaseFinancialModel, FinancialReport, TraceableField, XBRLProvenance, 
    SourceType, ManualProvenance, 
    IndustrialExtension, FinancialServicesExtension, RealEstateExtension
)
from .sec_extractor import SECReportExtractor, SearchType, SearchConfig

T = TypeVar("T")

class BaseFinancialModelFactory:
    @staticmethod
    def _extract_field(extractor: SECReportExtractor, 
                      configs: List[SearchConfig], 
                      name: str, 
                      target_type: Type[T] = float) -> TraceableField[T]:
        """
        ...
        return TraceableField(
            name=name,
            value=None,
            provenance=ManualProvenance(description=f"Not found in XBRL. Searched: {tags_searched}")
        )
    ...
    def create(extractor: SECReportExtractor) -> BaseFinancialModel:
        ...
        tf_ticker = TraceableField(name="Ticker", value=ticker_val, provenance=ManualProvenance(description="Input Ticker"))
        tf_cik = TraceableField(name="CIK", value=cik_val, provenance=XBRLProvenance(concept="dei:EntityCentralIndexKey", period="Current") if cik_val else ManualProvenance(description="Missing"))
        tf_name = TraceableField(name="Company Name", value=name_val, provenance=XBRLProvenance(concept="dei:EntityRegistrantName", period="Current") if name_val else ManualProvenance(description="Missing"))
        
        tf_sic = TraceableField(name="SIC Code", value=str(extractor.sic_code()), provenance=ManualProvenance(description="From Company Profile"))
        ...
        # Balance Sheet
        tf_assets = BaseFinancialModelFactory._extract_field(extractor, [C("us-gaap:Assets")], "Total Assets", target_type=float)
        tf_liabilities = BaseFinancialModelFactory._extract_field(extractor, [C("us-gaap:Liabilities")], "Total Liabilities", target_type=float)
        tf_equity = BaseFinancialModelFactory._extract_field(
             extractor, 
             [
                 C("us-gaap:StockholdersEquity"), 
                 C("us-gaap:StockholdersEquityIncludingPortionAttributableToNoncontrollingInterest")
             ], 
             "Total Equity",
             target_type=float
        )
        tf_cash = BaseFinancialModelFactory._extract_field(extractor, [C("us-gaap:CashAndCashEquivalentsAtCarryingValue")], "Cash & Cash Equivalents", target_type=float)
        ...
        tf_net_income = BaseFinancialModelFactory._extract_field(extractor, [C("us-gaap:NetIncomeLoss")], "Net Income", target_type=float)
        tf_tax = BaseFinancialModelFactory._extract_field(extractor, [C("us-gaap:IncomeTaxExpenseBenefit")], "Income Tax Expense", target_type=float)

        # Cash Flow
        tf_ocf = BaseFinancialModelFactory._extract_field(extractor, [C("us-gaap:NetCashProvidedByUsedInOperatingActivities")], "Operating Cash Flow (OCF)", target_type=float)
