...
TECHNICAL_DIRECTION_CALIBRATION_MAPPING_PATH_ENV = (
    "TECHNICAL_DIRECTION_CALIBRATION_MAPPING_PATH"
)
_DEFAULT_MAPPING_PATH = (
    Path(__file__).resolve().parent
    / "config"
    / "technical_direction_calibration.default.json"
)


@dataclass(frozen=True)
class TechnicalDirectionCalibrationLoadResult:
...
def load_technical_direction_calibration_mapping(
    mapping_path: str | None = None,
) -> TechnicalDirectionCalibrationLoadResult:
    return _load_technical_direction_calibration_mapping_cached(
        mapping_path or os.getenv(TECHNICAL_DIRECTION_CALIBRATION_MAPPING_PATH_ENV),
    )


def clear_technical_direction_calibration_mapping_cache() -> None:
    _load_technical_direction_calibration_mapping_cached.cache_clear()


@lru_cache(maxsize=4)
def _load_technical_direction_calibration_mapping_cached(
...
def _load_from_path(
    ...
    *,
    ...
    try:
        ...
        config = parse_technical_direction_calibration_config(payload)
        return TechnicalDirectionCalibrationLoadResult(
            config=config,
        ...
        return TechnicalDirectionCalibrationLoadResult(
            config=DEFAULT_TECHNICAL_DIRECTION_CALIBRATION_CONFIG,
