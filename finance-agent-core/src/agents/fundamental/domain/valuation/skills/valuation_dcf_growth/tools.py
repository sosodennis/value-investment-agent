...
def _validate_projection_lengths(params: DCFGrowthParams) -> str | None:
...
def _run_dcf_growth_monte_carlo(
    *,
    ...
    distributions: dict[str, DistributionSpec] = {
        "growth_shock": DistributionSpec(
            kind="normal",
            mean=0.0,
            std=max(params.growth_shock_std * 1.2, 0.01),
            min_bound=-0.35,
            max_bound=0.35,
        ),
        "margin_shock": DistributionSpec(
            kind="normal",
            mean=0.0,
            std=max(params.margin_shock_std * 1.2, 0.01),
            min_bound=-0.18,
            max_bound=0.18,
        ),
...
def calculate_dcf_growth_valuation(
    ...
    try:
        ...
        if params.monte_carlo_iterations > 0:
            details["distribution_summary"] = _run_dcf_growth_monte_carlo(
                params=params,
                converged_inputs={
