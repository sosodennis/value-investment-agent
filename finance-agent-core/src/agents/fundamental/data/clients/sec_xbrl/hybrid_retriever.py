...
class _DenseRetriever:
    ...
    def _get_or_encode_query_embedding(self, *, query: str) -> object | None:
        embeddings = self._get_or_encode_query_embeddings(queries=[query])
        if embeddings is None:
            return None
        return embeddings[0]
...
def _sparse_rank(*, query: str, corpus: list[str]) -> list[int]:
    rankings = _sparse_rank_many(queries=[query], corpus=corpus)
    if not rankings:
        return []
    return rankings[0]
