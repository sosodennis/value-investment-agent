...
from src.agents.technical.subdomains.calibration.domain.fitting_service import (  # noqa: E402
    fit_technical_direction_calibration_config,
)
from src.agents.technical.subdomains.calibration.domain.io_service import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fit technical direction calibration mapping artifact from offline "
    ...
    parser.add_argument(
        "--fit-timeframe-multipliers",
        action="store_true",
        help="Fit timeframe multipliers from data (else keep defaults).",
    )
    parser.add_argument(
        "--fit-direction-multipliers",
        action="store_true",
        help="Fit direction multipliers from data (else keep defaults).",
    )
    parser.add_argument(
        "--require-fit",
        action="store_true",
...
def main() -> int:
    ...
    load_result = load_technical_direction_calibration_observations(args.input)
    fit_result = fit_technical_direction_calibration_config(
        load_result.observations,
        mapping_version=mapping_version,
        min_samples=max(int(args.min_samples), 1),
        fit_timeframe_multipliers=bool(args.fit_timeframe_multipliers),
        fit_direction_multipliers=bool(args.fit_direction_multipliers),
    )

    artifact = {
        "mapping_version": fit_result.config.mapping_version,
        "timeframe_multiplier": fit_result.config.timeframe_multiplier,
        "direction_multiplier": fit_result.config.direction_multiplier,
        "mapping_bins": [
            [upper, confidence] for upper, confidence in fit_result.config.mapping_bins
        ],
    }
    write_technical_direction_calibration_artifact(
...
def _default_mapping_version() -> str:
    ...
    return f"technical_direction_calibration_fit_{now}"
