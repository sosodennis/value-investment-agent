# Fundamental Cohort Release Checklist (S14, prod profile)
...
- Backtest report path: `finance-agent-core/reports/fundamental_backtest_report_s14_prod.json`
- Replay report path: `finance-agent-core/reports/fundamental_replay_checks_report_s14_prod.json`
- Snapshot path: `finance-agent-core/reports/fundamental_release_gate_snapshot_s14_prod.json`
...
- Reason: all configured production profile thresholds satisfied; no blocking issue.
- Rollback artifact (if needed): previous approved mapping/profile artifacts.
- Follow-up actions:
  - replace fixture cohort with live production cohort sample and rerun checklist before external production promotion.
