...
from src.workflow.nodes.fundamental_analysis.factories import FinancialReportFactory, BaseFinancialModelFactory
from src.workflow.nodes.fundamental_analysis.sec_extractor import SECExtractResult, SECReportExtractor
from src.workflow.nodes.fundamental_analysis.financial_models import ComputedProvenance, ManualProvenance, XBRLProvenance, TraceableField
...
def test_real_estate_ffo_calculation(mock_extractor):
    ...
    base_model.net_income = TraceableField(
        name="Net Income",
        value=1000.0,
        provenance=ManualProvenance(description="Mock")
    )

    # Setup extractor for Depreciation and GainOnSale
    def search_side_effect(config):
        if "DepreciationAndAmortizationInRealEstate" in config.concept_regex:
            return [create_mock_result("us-gaap:DepreciationAndAmortizationInRealEstate", 200.0)]
        if "GainLossOnSaleOfRealEstateInvestmentProperty" in config.concept_regex:
            return [create_mock_result("us-gaap:GainLossOnSaleOfRealEstateInvestmentProperty", 50.0)]
        return []

    mock_extractor.search.side_effect = search_side_effect

    extension = FinancialReportFactory._create_real_estate_extension(mock_extractor, base_model)
