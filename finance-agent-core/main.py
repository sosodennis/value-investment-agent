...
def main():
    ...
    initial_input = {"user_query": user_query}
    ...
    if snapshot.next:
        ...
        if "planner_human_review" in snapshot.next:
            ...
            candidates = (
                state_values.ticker_candidates
                if hasattr(state_values, "ticker_candidates")
                else state_values.get("ticker_candidates", [])
            )
            ...
            if candidates:
                ...
                for idx, candidate in enumerate(candidates):
                    ...
                    symbol = (
                        ...
                        else candidate.get("symbol", "N/A")
                    )
                    name = (
                        candidate.name
                        if hasattr(candidate, "name")
                        else candidate.get("name", "N/A")
                    )
                    exchange = (
                        candidate.exchange
                        if hasattr(candidate, "exchange")
                        else candidate.get("exchange", "N/A")
                    )
                    confidence = (
                    ...
                    print(
                        f"  [{idx}] {symbol} - {name} ({exchange}) - Confidence: {confidence:.2f}"
                    )

                # Ask user to select
                selection = input(
                    "\nEnter the number of your choice (or press Enter to use default): "
                ).strip()
                ...
                if selection.isdigit() and 0 <= int(selection) < len(candidates):
                    ...
                    selected_symbol = (
                        ...
                        if hasattr(selected_candidate, "symbol")
        ...
        # Check if this is a calculator interrupt (parameter approval)
            ...
            print("\n=== Parameter Review ===")
            extraction_output = (
                state_values.extraction_output
                if hasattr(state_values, "extraction_output")
                else state_values.get("extraction_output")
            )
