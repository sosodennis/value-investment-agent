...
const SourceBadge = ({
    ...
    if (type === 'fact') {
        ...
        return (
            <span
                ...
                className={`inline-flex items-center px-2 py-0.5 rounded border text-[9px] font-black tracking-widest mx-1 cursor-pointer transition duration-200 align-middle shadow-sm hover:scale-105 active:scale-95 ${color}`}
            >
    ...
    return (
        <span className={`inline-flex items-center px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-widest mx-1 cursor-pointer transition-colors duration-200 align-middle shadow-sm ${color}`}>
            <span className="mr-1.5 opacity-80">{icon}</span>
            {source}
        </span>
    );
