        ...
        <div className="flex flex-col h-full animate-in slide-in-from-bottom-2 duration-300">
            {agentMessages.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
                    <MessageSquare size={48} className="text-outline mb-4 opacity-50" />
                    <h4 className="text-[10px] uppercase font-black tracking-widest text-outline">No activity yet</h4>
                    <p className="text-on-surface-variant text-[10px] mt-2 max-w-[200px]">This agent has not generated any messages or outputs for this session yet.</p>
                </div>
            ) : (
                <div className="p-8 space-y-6">
                    {agentMessages.map((msg, i) => (
                        <div key={msg.id || i} className={`group flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                            <div className={`max-w-[85%] rounded-2xl p-4 text-sm leading-relaxed border ${msg.role === 'user' ? 'bg-primary/10 border-primary/20 text-on-surface' : 'bg-surface-container-low border-outline-variant/30 text-on-surface-variant'}`}>
                                {msg.content}
                                ...
                                {msg.type === 'financial_report' && (
                                    <div className="mt-4 p-4 bg-surface-container rounded-xl border border-outline-variant/30">
                                        <div className="flex items-center gap-2 mb-3 text-primary">
                                            <BarChart3 size={14} />
                                            <span className="text-[10px] font-black uppercase tracking-widest text-primary">Extracted Financials</span>
                                        </div>
                            ...
                            <span className="mt-2 px-2 text-[9px] font-bold text-outline uppercase tracking-widest">{msg.role}</span>
                        </div>
                    ))}
                </div>
            )}
