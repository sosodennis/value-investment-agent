...
- Rework fusion runtime math or introduce new market data dependencies.
...
- Keep fusion/runtime scoring logic unchanged in this task.
...
- Deterministic projection logic that derives these summaries from existing raw confidence, quality, degraded, observability, and conflict signals.
...
1. Open a Technical report where `confidence_raw` is high but `calibration_applied=false`.
2. Confirm the overview shows `Signal Strength` and `Setup Reliability`, not a standalone primary `Confidence` probability.
3. Confirm degraded/conflict/calibration context is surfaced in user-facing reliability messaging.
